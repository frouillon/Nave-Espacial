...
class Nave
{
	...
	int corazones;
	public:
	Nave(int a, int b, int _corazones);
...
Nave::Nave(int a, int b, int _corazones)
{
	x=a;
	y=b;
...
{
	if ((N.getX() + 4 >= x && N.getX() <= x && N.getY() <= y + 0 && N.getY() + 3 >= y)&& (N.vidas()<4 && N.vidas()>1))
	{
		N.menos_corazones();
		N.eliminar();
		N.pintar();
	...
	if ((N.getX() + 4 >= x && N.getX() <= x && N.getY() <= y + 0 && N.getY() + 3 >= y) && N.vidas()==1)
	{
		N.mas_corazones();
		N.eliminar();
		N.pintar();
	...
	do{
		...
		switch(option){
			case 1:{
			    ...
			    Nave n(30,15,3);		//Se imprime la primera posicion de la nave
